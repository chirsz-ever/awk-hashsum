#!/bin/sh -e

null() { case $1 in '') return 0; esac; return 1; }

usage() {
    echo "Usage: $(basename "$0") [--tag] [FILE]..."
    echo "Calculate the MD5 checksum of the given files."
    echo "If no files are given, or argument is -, read from standard input."
}

# process options
bsd_foramt=0
for arg in "$@"; do
    case $arg in
    -)  continue;;
    --) break;;
    --help|-h)
        usage
        exit 0;;
    --tag)
        bsd_foramt=1;;
    -*)
        echo "Unknown option: $arg" >&2
        echo "Use --help for usage information." >&2
        exit 1;;
    esac
done

# use built-in bitwise functions if available.
sed_cmd='1,/^#_AWK_BEGIN_$/d'
sed_cmd_buitin_bitwise='/^#_POSIX_BEGIN_$/,/^#_POSIX_END_$/d'
ty=
case $(awk --version 2>&1 | cat) in
*'GNU Awk'*) ty=gnu;;
*BusyBox*) ty=busybox;;
esac
case $ty in
gnu)
    # if POSIXLY_CORRECT is not an environment variable, or is empty
    if ! { env | grep -q '^POSIXLY_CORRECT='; } || null "${POSIXLY_CORRECT-}"; then
        sed_cmd=$sed_cmd\;$sed_cmd_buitin_bitwise
    fi;;
busybox)
    # BusyBox awk always uses built-in bitwise functions.
    sed_cmd=$sed_cmd\;$sed_cmd_buitin_bitwise
esac

awk_script="$(sed "$sed_cmd" "$0")"
run_once=
run_once_on() {
    if [ "$1" != '-' ] && [ ! -r "$1" ]; then
        echo "Cannot read file: $1" >&2
        exit 1
    fi
    od -v -A n -t u1 -- "$1" | awk -v fname="$1" -v bsd_foramt="$bsd_foramt" "$awk_script"
    run_once=1
}

options_end=
for arg do
    case $options_end in
    1)
        run_once_on "$arg"
        continue
    esac

    case $arg in
    -)  run_once_on -;;
    --) options_end=1;;
    -*) continue;;
    *)  run_once_on "$arg";;
    esac
done
case $run_once in '')
    # no files given, read from stdin.
    run_once_on -
esac
exit $?

# a trick to suppress shellcheck warnings.
# shellcheck disable=SC2317
: <<'#_AWK_END_'
#_AWK_BEGIN_

BEGIN {
#_POSIX_BEGIN_
    _bitwise_init();
#_POSIX_END_
    _md5_init();
}

# globals: state, content_bytes, bytes_count

{
    # Every 512 bits/64 bytes, run a round.
    # This avoid memory issues with large files.
    for (i = 1; i <= NF; ++i) {
        push_byte($(i));
    }
}

END {
    run_pad();

    if (bsd_foramt) {
        printf("MD5 (%s) = %s\n", fname, md5());
    } else {
        printf("%s  %s\n", md5(), fname);
    }
}

# our md5 implementation
function md5(i, j, ret, digest) {

    for (i = j = 0; j < 16; j += 4) {
        digest[j]     = state[i] % (2 ^ 8);
        digest[j + 1] = rshift(state[i],  8) % (2 ^ 8);
        digest[j + 2] = rshift(state[i], 16) % (2 ^ 8);
        digest[j + 3] = rshift(state[i], 24) % (2 ^ 8);
        ++i;
    }
    for (i = 0; i < 16; i++)
        ret = sprintf("%s%02x", ret, digest[i]);
    return ret;
}

function push_byte(b) {
    content_bytes[bytes_count % 64 + 1] = b;
    ++bytes_count;
    if (bytes_count % 64 == 0)
        run_round();
}

function run_round(i, hi, lo, nwords, a, b, c, d) {
    # convert the array of bytes into an array of 32-bits words
    for (i = 1; i <= 64; i += 4) {
        hi = lshift(content_bytes[i + 3], 8) + content_bytes[i + 2];
        lo = lshift(content_bytes[i + 1], 8) + content_bytes[i + 0];
        x[nwords++] = lshift(hi, 16) + lo;
    }

    a = state[0]; b = state[1]; c = state[2]; d = state[3];

    # Round 1
    a = FF(a, b, c, d, x[ 0], S11, 3614090360); # 0xd76aa478
    d = FF(d, a, b, c, x[ 1], S12, 3905402710); # 0xe8c7b756
    c = FF(c, d, a, b, x[ 2], S13,  606105819); # 0x242070db
    b = FF(b, c, d, a, x[ 3], S14, 3250441966); # 0xc1bdceee
    a = FF(a, b, c, d, x[ 4], S11, 4118548399); # 0xf57c0faf
    d = FF(d, a, b, c, x[ 5], S12, 1200080426); # 0x4787c62a
    c = FF(c, d, a, b, x[ 6], S13, 2821735955); # 0xa8304613
    b = FF(b, c, d, a, x[ 7], S14, 4249261313); # 0xfd469501
    a = FF(a, b, c, d, x[ 8], S11, 1770035416); # 0x698098d8
    d = FF(d, a, b, c, x[ 9], S12, 2336552879); # 0x8b44f7af
    c = FF(c, d, a, b, x[10], S13, 4294925233); # 0xffff5bb1
    b = FF(b, c, d, a, x[11], S14, 2304563134); # 0x895cd7be
    a = FF(a, b, c, d, x[12], S11, 1804603682); # 0x6b901122
    d = FF(d, a, b, c, x[13], S12, 4254626195); # 0xfd987193
    c = FF(c, d, a, b, x[14], S13, 2792965006); # 0xa679438e
    b = FF(b, c, d, a, x[15], S14, 1236535329); # 0x49b40821

    # Round 2
    a = GG(a, b, c, d, x[ 1], S21, 4129170786); # 0xf61e2562
    d = GG(d, a, b, c, x[ 6], S22, 3225465664); # 0xc040b340
    c = GG(c, d, a, b, x[11], S23,  643717713); # 0x265e5a51
    b = GG(b, c, d, a, x[ 0], S24, 3921069994); # 0xe9b6c7aa
    a = GG(a, b, c, d, x[ 5], S21, 3593408605); # 0xd62f105d
    d = GG(d, a, b, c, x[10], S22,   38016083); # 0x2441453
    c = GG(c, d, a, b, x[15], S23, 3634488961); # 0xd8a1e681
    b = GG(b, c, d, a, x[ 4], S24, 3889429448); # 0xe7d3fbc8
    a = GG(a, b, c, d, x[ 9], S21,  568446438); # 0x21e1cde6
    d = GG(d, a, b, c, x[14], S22, 3275163606); # 0xc33707d6
    c = GG(c, d, a, b, x[ 3], S23, 4107603335); # 0xf4d50d87
    b = GG(b, c, d, a, x[ 8], S24, 1163531501); # 0x455a14ed
    a = GG(a, b, c, d, x[13], S21, 2850285829); # 0xa9e3e905
    d = GG(d, a, b, c, x[ 2], S22, 4243563512); # 0xfcefa3f8
    c = GG(c, d, a, b, x[ 7], S23, 1735328473); # 0x676f02d9
    b = GG(b, c, d, a, x[12], S24, 2368359562); # 0x8d2a4c8a

    # Round 3
    a = HH(a, b, c, d, x[ 5], S31, 4294588738); # 0xfffa3942
    d = HH(d, a, b, c, x[ 8], S32, 2272392833); # 0x8771f681
    c = HH(c, d, a, b, x[11], S33, 1839030562); # 0x6d9d6122
    b = HH(b, c, d, a, x[14], S34, 4259657740); # 0xfde5380c
    a = HH(a, b, c, d, x[ 1], S31, 2763975236); # 0xa4beea44
    d = HH(d, a, b, c, x[ 4], S32, 1272893353); # 0x4bdecfa9
    c = HH(c, d, a, b, x[ 7], S33, 4139469664); # 0xf6bb4b60
    b = HH(b, c, d, a, x[10], S34, 3200236656); # 0xbebfbc70
    a = HH(a, b, c, d, x[13], S31,  681279174); # 0x289b7ec6
    d = HH(d, a, b, c, x[ 0], S32, 3936430074); # 0xeaa127fa
    c = HH(c, d, a, b, x[ 3], S33, 3572445317); # 0xd4ef3085
    b = HH(b, c, d, a, x[ 6], S34,   76029189); # 0x4881d05
    a = HH(a, b, c, d, x[ 9], S31, 3654602809); # 0xd9d4d039
    d = HH(d, a, b, c, x[12], S32, 3873151461); # 0xe6db99e5
    c = HH(c, d, a, b, x[15], S33,  530742520); # 0x1fa27cf8
    b = HH(b, c, d, a, x[ 2], S34, 3299628645); # 0xc4ac5665

    # Round 4
    a = II(a, b, c, d, x[ 0], S41, 4096336452); # 0xf4292244
    d = II(d, a, b, c, x[ 7], S42, 1126891415); # 0x432aff97
    c = II(c, d, a, b, x[14], S43, 2878612391); # 0xab9423a7
    b = II(b, c, d, a, x[ 5], S44, 4237533241); # 0xfc93a039
    a = II(a, b, c, d, x[12], S41, 1700485571); # 0x655b59c3
    d = II(d, a, b, c, x[ 3], S42, 2399980690); # 0x8f0ccc92
    c = II(c, d, a, b, x[10], S43, 4293915773); # 0xffeff47d
    b = II(b, c, d, a, x[ 1], S44, 2240044497); # 0x85845dd1
    a = II(a, b, c, d, x[ 8], S41, 1873313359); # 0x6fa87e4f
    d = II(d, a, b, c, x[15], S42, 4264355552); # 0xfe2ce6e0
    c = II(c, d, a, b, x[ 6], S43, 2734768916); # 0xa3014314
    b = II(b, c, d, a, x[13], S44, 1309151649); # 0x4e0811a1
    a = II(a, b, c, d, x[ 4], S41, 4149444226); # 0xf7537e82
    d = II(d, a, b, c, x[11], S42, 3174756917); # 0xbd3af235
    c = II(c, d, a, b, x[ 2], S43,  718787259); # 0x2ad7d2bb
    b = II(b, c, d, a, x[ 9], S44, 3951481745); # 0xeb86d391

    state[0] = (state[0] + a) % (2 ^ 32);
    state[1] = (state[1] + b) % (2 ^ 32);
    state[2] = (state[2] + c) % (2 ^ 32);
    state[3] = (state[3] + d) % (2 ^ 32);
}

function run_pad(nbits, k) {
    nbits = bytes_count * 8;
    push_byte(128); # 0x80
    # fill the remaining bytes with 0 until we're just shy two words of
    # having 16-Word Blocks.
    while ((bytes_count % 64) != 56) {
        push_byte(0);
    }
    # append the length of the input in bits, as little-endian 64-bit integer.
    k = bytes_count % 64;
    content_bytes[++k] = nbits % 256;
    content_bytes[++k] = rshift(nbits,  8) % 256;
    content_bytes[++k] = rshift(nbits, 16) % 256;
    content_bytes[++k] = rshift(nbits, 24) % 256;
    content_bytes[++k] = rshift(nbits, 32) % 256;
    content_bytes[++k] = rshift(nbits, 40) % 256;
    content_bytes[++k] = rshift(nbits, 48) % 256;
    content_bytes[++k] = rshift(nbits, 56) % 256;
    bytes_count += 8;
    run_round();
}

function F(x, y, z) {
    return or(and(x, y), and(compl32(x), z));
}

function G(x, y, z) {
    return or(and(x, z), and(y, compl32(z)));
}

function H(x, y, z) {
    return xor(x, xor(y, z));
}

function I(x, y, z) {
    return xor(y, or(x, compl32(z))) % (2 ^ 32);
}

function FF(a, b, c, d, x, s, ac) {
    a = (a + F(b, c, d) + x + ac) % (2 ^ 32);
    a = ROTATE_LEFT(a, s);
    a = (a + b) % (2 ^ 32);
    return a;
}

function GG(a, b, c, d, x, s, ac) {
    a = (a + G(b, c, d) + x + ac) % (2 ^ 32);
    a = ROTATE_LEFT(a, s);
    a = (a + b) % (2 ^ 32);
    return a;
}

function HH(a, b, c, d, x, s, ac) {
    a = (a + H(b, c, d) + x + ac) % (2 ^ 32);
    a = ROTATE_LEFT(a, s);
    a = (a + b) % (2 ^ 32);
    return a;
}

function II(a, b, c, d, x, s, ac) {
    a = (a + I(b, c, d) + x + ac) % (2 ^ 32);
    a = ROTATE_LEFT(a, s);
    a = (a + b) % (2 ^ 32);
    return a;
}

function ROTATE_LEFT(x, n,    l, r) {
    l = lshift(x, n) % (2 ^ 32);
    r = rshift(x, 32 - n);
    return (r + l);
}

function _md5_init() {
    # MD5 shift constants setup.
    S11 =  7; S12 = 12; S13 = 17; S14 = 22;
    S21 =  5; S22 =  9; S23 = 14; S24 = 20;
    S31 =  4; S32 = 11; S33 = 16; S34 = 23;
    S41 =  6; S42 = 10; S43 = 15; S44 = 21;
    
    # Initialize MD Buffer
    state[0] = 1732584193; # 0x67452301
    state[1] = 4023233417; # 0xefcdab89
    state[2] = 2562383102; # 0x98badcfe
    state[3] =  271733878; # 0x10325476
}

# Busybox's built-in `compl` has a problem, so we use xor(x, 0xffffffff) instead.
# see https://lists.busybox.net/pipermail/busybox/2022-September/089902.html
function compl32(x) {
    return xor(x, 4294967295); # 0xffffffff
}

#_POSIX_BEGIN_

function lshift(x, n) {
    return x * (2 ^ n);
}

function rshift(x, n) {
    return int(x / (2 ^ n));
}

function and(x, y,    i, r) {
    for (i = 0; i < 32; i += 4) {
        r = r / (2 ^ 4) + lookup["and", x % 16, y % 16] * (2 ^ 28);
        x = int(x / (2 ^ 4));
        y = int(y / (2 ^ 4));
    }
    return r;
}

function or(x, y,    i, r) {
    for (i = 0; i < 32; i += 4) {
        r = r / (2 ^ 4) + lookup["or", x % 16, y % 16] * (2 ^ 28);
        x = int(x / (2 ^ 4));
        y = int(y / (2 ^ 4));
    }
    return r;
}

function xor(x, y) {
    return (x + y - 2 * and(x, y));
}

function _bitwise_init(    a, b, x, y, i) {
    # generate the lookup table used by and() and or().
    for (a = 0; a < 16; a++) {
        for (b = 0; b < 16; b++) {
            x = a;
            y = b;
            for (i = 0; i < 4; i++) {
                lookup["and", a, b] += ((x % 2) && (y % 2)) * (2 ^ i);
                lookup["or",  a, b] += ((x % 2) || (y % 2)) * (2 ^ i);
                x = int(x / 2);
                y = int(y / 2);
            }
        }
    }
}

#_POSIX_END_
#_AWK_END_
