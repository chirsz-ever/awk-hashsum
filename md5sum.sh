...
function md5(bytes, nbytes, chars, i, hi, lo, words, nwords, state,
    ...
    for (i = 1; i <= nbytes; i += 4) {
        hi = lshift(bytes[i + 3], 8) + bytes[i + 2];
        lo = lshift(bytes[i + 1], 8) + bytes[i + 0];
        words[nwords++] = lshift(hi, 16) + lo;
    }
    ...
    if (nbytes % 4 == 0) {
        ...
        words[nwords - 1] = words[nwords - 1] + lshift(128, (nbytes % 4) * 8); # 0x80
    }
